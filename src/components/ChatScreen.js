...
import {usersCollection, chatsCollection} from '../services/firebase'
...
const ChatScreen = ({match}) => {
    ...
    useEffect(() => {
        ...
        const docRef = chatsCollection.doc(currentUser.uid).collection('users').doc(personId);
        ...
        usersCollection.doc(personId).get().then(doc => {
    ...
    const handleSend = e => {
        ...
        if(input.length) {
            ...
            chatsCollection.doc(currentUser.uid).collection('users').doc(personId).update({messages: firebase.firestore.FieldValue.arrayUnion(newMessage)});
            chatsCollection.doc(personId).collection('users').doc(currentUser.uid).update({messages: firebase.firestore.FieldValue.arrayUnion(newMessage)});
            setInput('');
        }
    }
